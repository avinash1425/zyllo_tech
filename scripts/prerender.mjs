...
async function main() {
  ...
  // Body text is Inter 400, and its filename is content-hashed by the build,
  // so the preload can't live in index.html — without it the font request only
  // starts after the CSS is fetched and parsed, so the first paint is in a
  // fallback face that then swaps. Preload the one weight that covers body
  // copy; the heavier weights can swap without being noticed.
  let fontPreload = "";
  try {
  ...
  const CORE = [
    ...
    { p: "/hire-dedicated-developers", label: "Hire Dedicated Developers", t: "Hire Dedicated Developers in India | Zyllo Tech", d: "Hire senior React, Node, Python, Flutter, AI/LLM, QA and DevOps engineers from India who join your workflow, demo weekly, and own no lock-in.", h1: "Hire Dedicated Developers Who Ship Weekly" },
    { p: "/engagement-models", label: "Engagement Models", t: "Software Development Engagement Models | Zyllo Tech", d: "Fixed scope, dedicated team, or staff augmentation — an honest comparison of software development engagement models and when each one fits.", h1: "Engagement Models: Fixed Scope, Dedicated Team, or Staff Augmentation" },
    { p: "/software-development-company-usa", label: "For US Companies", t: "Offshore Software Development for US Companies | Zyllo Tech", d: "India-based software development partner for US companies: 4+ hours of East Coast overlap, US-style contracts with full IP assignment, weekly demos.", h1: "Software Development Partner for US Companies" },
    { p: "/software-development-company-europe", label: "For UK/EU Companies", t: "Software Development for UK & EU Companies | Zyllo Tech", d: "India-based software development partner for UK and EU companies: full CET business-hours overlap, GDPR-aware engineering with DPAs, weekly demos.", h1: "Software Development Partner for European Companies" },
  ...
  for (const page of CORE) {
    ...
    // FAQPage markup must mirror rendered content, so the visible Q&A and the
    ...
    if (faqs?.length) {
      body += `<section><h2>Common questions</h2>${faqs
        ...
        .join("")}</section>`;
      if (faqSchema) schemas.push(faqSchema(faqs));
    }
