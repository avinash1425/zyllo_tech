...
export const articles: Article[] = [
  // ─── Existing Blog Articles (with slugs) ───────────────────────────────────
  {
    ...
    content: [
      ...
      { type: "flow", title: "What a data readiness audit actually checks, before any model is chosen", steps: [
        "Lineage — can you say where each field came from and what transformed it? Without this, a wrong model output cannot be traced to a cause, and you end up retraining on a hunch.",
        "Labels — do ground-truth labels exist, who produced them, and do two labellers agree? Disagreement between humans is the ceiling on model accuracy, so measure it before promising a number.",
        "Coverage and bias — which cohorts are thin or missing? A model is confidently wrong exactly where the training data was sparse, and that is rarely the cohort you tested with.",
        "Freshness and drift — how old is the data at inference time, and how fast does the world it describes change? A model trained on last quarter's behaviour needs a monitored decay assumption, not an annual retrain by calendar.",
        "Governance — what is personal, what is regulated, what may leave your infrastructure? Decide this before a vendor API is in the request path, not after.",
      ] },
      { type: "h2", text: "How do you keep an LLM feature from inventing answers?" },
      { type: "p", text: "The single highest-value pattern in production LLM work is not a better prompt — it is refusing to answer without grounding, and making the model's confidence legible to the caller. That is a code-level contract, not a wording choice." },
      { type: "code", lang: "python", label: "Grounded answering: refuse rather than guess", code: "def answer(question: str, k: int = 6) -> dict:\n    chunks = retrieve(question, k=k)\n\n    # 1. No evidence, no answer. This branch is the whole feature: a system\n    #    that says \"I don't know\" is trusted; one that improvises is not.\n    if not chunks or top_score(chunks) < RELEVANCE_FLOOR:\n        return {\"status\": \"no_answer\", \"reason\": \"no sufficiently relevant source\"}\n\n    reply = llm(\n        system=\"Answer ONLY from the context. If the context does not \"\n               \"contain the answer, say so. Cite the chunk ids you used.\",\n        context=chunks,\n        question=question,\n    )\n\n    # 2. Verify the citations point at chunks actually retrieved — a model\n    #    can cite plausibly and still be citing nothing.\n    cited = [c for c in reply.citations if c in {c.id for c in chunks}]\n    if not cited:\n        return {\"status\": \"unverified\", \"draft\": reply.text}\n\n    return {\"status\": \"ok\", \"text\": reply.text, \"sources\": cited}\n\n# Log every branch. The ratio of ok / unverified / no_answer over time is\n# the only honest health metric for a RAG feature — accuracy claims made\n# without it are anecdotes." },
  ...
  },
  {
    ...
    content: [
      ...
      { type: "flow", title: "Where RAG quality is actually won or lost — the same five stages in both frameworks", steps: [
        "Chunking — split documents so each chunk is a self-contained idea. Splitting mid-table or mid-clause is the most common root cause of confidently wrong answers, and no amount of prompt engineering downstream repairs it.",
  ...
  },
  {
    ...
    content: [
      ...
      },
      {
        type: "code",
        lang: "json",
        label: "Product → variant → SKU, with attributes at the level where they actually vary",
      ...
      {
        type: "flow",
        title: "Selling the last unit exactly once, with 40,000 people trying to buy it",
        steps: [
  ...
  },
  {
    ...
    content: [
      ...
      },
      {
        type: "code",
        lang: "text",
        label: "The HLS master playlist — the bitrate ladder a learner's player chooses from",
      ...
      {
        type: "flow",
        title: "What happens between an instructor's upload and a learner pressing play",
        steps: [
          "The upload lands in object storage and is never served from there. Keep the original: it is the master you re-encode from the next time the ladder changes.",
  ...
  },
  {
    ...
    content: [
      ...
      {
        type: "flow",
        title: "From a GPS ping to a moving dot on the dispatcher's map",
        steps: [
          "The device publishes to its own topic at QoS 1, buffering locally whenever the network drops. A vehicle leaving a tunnel replays its backlog, so arrival order is not chronological order — every stage downstream has to assume this.",
          "The broker authenticates the device by certificate and forwards to the ingestion stream. Payloads failing schema validation go to a dead-letter topic, so a bad firmware rollout is visible as a spike rather than as silence.",
          "Hot path: the device shadow takes the newest position by device timestamp, not arrival time, and pushes to dashboards over WebSocket. Ordering by arrival is what makes a replayed old ping drag the dot backwards across the map.",
          "Stream path: windowed aggregations, geofence evaluation and trip segmentation run off the same stream, using a watermark to admit late data instead of assuming it arrives in order.",
          "Cold path: raw pings land in time-series and object storage for history, ETA model training and delivery disputes — the one place where full fidelity matters more than latency.",
        ],
      },
      {
        type: "ul",
  ...
  },
  {
    ...
    content: [
      ...
      {
        type: "flow",
        title: "One shot, from key-press to authoritative hit",
        steps: [
  ...
  },
  {
    ...
    content: [
      ...
      },
      {
        type: "code",
        lang: "text",
        label: "The unit lifecycle, written as transitions and guards rather than a status column",
        code: "STATE              ALLOWED NEXT        GUARD\navailable       -> held               hold TTL set, agent assigned\nheld            -> available          TTL expired (automatic, never manual)\nheld            -> booked             booking amount received\nbooked          -> agreement_signed   KYC complete, agreement executed\nbooked          -> available          cancellation, with reason recorded\nagreement_signed-> handed_over        all milestone payments cleared\n\n# Why this belongs in the platform rather than in a status dropdown:\n# - An expired hold must return the unit to available on its own. If\n#   releasing it depends on someone remembering, inventory silently\n#   locks up and the sales team goes back to a spreadsheet.\n# - Every transition records who, when and why. Across a 3-5 year\n#   project the audit trail is the dispute defence.\n# - Demand letters fire on state plus construction milestone, never on\n#   a calendar date — so a schedule slip cannot bill a buyer for work\n#   that has not happened yet.",
      },
      {
        type: "h2",
  ...
  },
  {
    ...
    content: [
      ...
      {
        type: "flow",
        title: "How one data session becomes one line on an invoice",
        steps: [
